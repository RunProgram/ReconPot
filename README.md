...
## 🚀 Features
- Recon Jammer's most important feature is silent redirection which is achieved via Apache2's ```mod_rewrite``` instead of classic 301 redirection. This means that both standard users (real users, administrators, etc.) and attackers will see the ```/admin``` page. However, depending on if your IP has tripped the honeypot you may get a poisoned version, even at the exact same directory.
